...
public class PostNewsHandler : AbstractHandler
{
    private readonly IPostReadRepository repository;
    private readonly IUsersSubscriptionReadService _usersSubscriptionService;

    public PostNewsHandler(IPostReadRepository repository, IUsersSubscriptionReadService usersSubscriptionService, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
    {
    ...
    public override ListResponseBase<PostDto> Handle(object request,User currentUser)
    {
        ...
        if ((GetPostType)request == GetPostType.News)
        {
            var result= repository.News(currentUser, isRemoveAds);
            return new(result);
        }
        else
        {
