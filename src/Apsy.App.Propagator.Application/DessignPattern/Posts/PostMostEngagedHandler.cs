...
public class PostMostEngagedHandler : AbstractHandler
{
    private readonly IPostReadRepository repository;
    private readonly IUsersSubscriptionReadService _usersSubscriptionService;

    public PostMostEngagedHandler(IPostReadRepository repository, IUsersSubscriptionReadService usersSubscriptionService, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
    {
    ...
    public override ListResponseBase<PostDto> Handle(object request,User currentUser)
    {
        ...
        if ((GetPostType)request == GetPostType.MostEngaged)
        {
            var result = repository.MostEngaged(currentUser, isRemoveAds);
            return new(result);
