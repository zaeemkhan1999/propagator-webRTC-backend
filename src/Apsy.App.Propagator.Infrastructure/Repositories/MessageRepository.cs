...
public class MessageRepository
 : MessageRepositoryBase<DataReadContext, Message, Conversation, User>, IMessageRepository
{
    public MessageRepository(IDbContextFactory<DataReadContext> dbContextFactory)
    ...
    public IQueryable<Message> GetMessages(int Id, bool checkeddelete = false)
    {
        if (checkeddelete)
        {
         ...
         return context.Message.Include(x=>x.Conversation).Where(x=>x.Id==Id);
    }
